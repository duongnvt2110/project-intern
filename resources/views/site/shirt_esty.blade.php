...
<div class='row items-row'>
  ...
  <div class="items-list col-sm-3 col-md-3 col-xs-12">
    ...
    <div class='export-csv name-tag'>
       <input type="checkbox" class="export-csv" name="export-csv" value="{{$value['id']}}">&nbsp;Export CSV<br>
     </div>
    <div class="bottom">
              ...
              <div class="name-tag">
                <a class="Name"  @if($value['is_export']==true)
                      style='color: red';
                @endif 
                target="_blank" href="{{$value['url_product']}}">{{$value['title']}}</a>
              </div>
